...
int match_statement(list<QString> &token_types, list<QString> &token_Value, list<QString> ::iterator &it_types, list<QString> ::iterator &it_values, int &length, node *&input);
                            ...
                            if (temp_length !=token_types.size())
                            {
                                ...
                                if (match_statement_seq(token_types, token_Value, it_temp_type, it_temp_value, temp_length, then_part))
                                {
                                    if (temp_length !=token_types.size())
                                    {
                                        ...
                                        else if (*it_temp_type == "END")  // modified from else if to if
                                        {
                                            input->insert_chlid(test);
                                            input->insert_chlid(then_part);
                                            it_temp_type++;
...
bool match_statement_seq(list<QString> &token_types, list<QString> &token_Value, list<QString> ::iterator &it_types, list<QString> ::iterator &it_values,int &length, node * &input)
{
    if (match_statement(token_types, token_Value, it_types, it_values, length,input))
    {
        ...
        if (length !=token_types.size())
        {
            ...
            {
                //cout<<*it_types;
                int i=match_statement(token_types, token_Value, it_types, it_values, length,input);
...
void Widget::setputUi()
{
       ...
       QVBoxLayout  *horizontalforinput= new QVBoxLayout;
       QVBoxLayout  *horizontalforoutput= new QVBoxLayout;
       QVBoxLayout  *vertical= new QVBoxLayout;
       QHBoxLayout * horizontalforbutton = new QHBoxLayout;
       QVBoxLayout * submithandel = new QVBoxLayout;
        QVBoxLayout * openfilehandel = new QVBoxLayout;
       QVBoxLayout * parsetreehandel = new QVBoxLayout;
       QVBoxLayout * horizontalforinputlabel = new QVBoxLayout;
       QVBoxLayout *horizontalforoutputlabel = new QVBoxLayout;
       label= new QLabel;
       ...
       outputlabel->setText("Scanner Output");
       ...
       open_file->setSizePolicy(QSizePolicy::Fixed,QSizePolicy::Fixed);
       show_perse_tree->setSizePolicy(QSizePolicy::Fixed,QSizePolicy::Fixed);
       show_perse_tree->setEnabled(false);
       horizontalforinputlabel->addWidget(inputlabel);
       horizontalforinput->addWidget(inputbox);
       horizontalforoutputlabel->addWidget(outputlabel);

       horizontalforoutput->addWidget(output);
       //horizontal->addWidget(submit);
       horizontalforinput->addStretch();
       horizontalforoutput->addStretch();
       horizontalforoutput->setStretchFactor(output,1);
       horizontalforinput->setStretchFactor(inputbox,1);
       //QScrollArea *tm = new QScrollArea(this);
       submithandel->addWidget(submit);
       parsetreehandel->addWidget(show_perse_tree);
       openfilehandel->addWidget(open_file);
      ...
      // horizontalforbutton->setSpacing(200);
       ...
       openfilehandel->setAlignment(Qt::AlignCenter);
       ...
       horizontalforbutton->addLayout(openfilehandel);
       //vertical->addWidget(label);

        vertical->addLayout(horizontalforbutton);
        vertical->addLayout( horizontalforinputlabel);
//        vertical->addLayout(horizontal);
        vertical->addLayout(horizontalforinput);
        vertical->addLayout(horizontalforoutputlabel);
        vertical->addLayout(horizontalforoutput);
       vertical->addStretch();
      // vertical->setStretchFactor(label,1);
       vertical->setStretchFactor(horizontalforinput,1);
       vertical->setStretchFactor(horizontalforoutput,1);
       ...
       connect(submit,SIGNAL(clicked()),this,SLOT(button_clicked()));
        connect(open_file,SIGNAL(clicked()),this,SLOT(open_file_clicked()));
...
void Widget::open_file_clicked()
{
      ...
      QFile input_file(QFileDialog::getOpenFileName(this,tr("Open File"),"",tr("TextFile (*.txt)")));
     ...
     inputbox->setLineWrapMode(QTextEdit::NoWrap);
